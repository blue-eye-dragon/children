            ...
            <ta-row>
              <ta-col :span="12">
                <ta-form-item label="补领原因"
                              ...
                              :fieldDecoratorOptions="{rules: [{ required: true, message: '请选择补领原因'}]}">
              ...
              <ta-col :span="12">
                ...
                <ta-form-item label="登记证字号"
                              fieldDecoratorId="adoptionRegistionCertificateNumber"
                              :require="{message:'请输入原登记证字号!'}">
                  <ta-input placeholder="请输入原登记证字号"
                            :disabled="true" />
                </ta-form-item>
              </ta-col>
              <ta-col :span="24">
...
export default {
  ...
  data () {
    return {
      ...
      showfour: true,
      rules: {
        renewPrintNumber: this.verificationRules('补发证件印制号', 50),
        opinion: this.verificationRules('审核意见', 200, true),
        remarks: this.verificationRules('备注', 200, false),
        renewCertificateNumber: this.verificationRules('补发证件字号', 50),
  ...
  methods: {
    ...
    callback (key) {
      this.nowkey = key
      if (this.nowkey == '5') { this.setHeight('four', 'fourout') }
    },
    loadData () {
    ...
    async getdetails () {
      ...
      const data = await this.post('/adoption/queryRenewAdoptionById', params)
    ...
    async submit () {
      this.from.validateFields(async (err, values) => {
        if (!err) {
          ...
          if (this.otherid) {
            // 编辑
            url = '/adoption/renewUpdateAdoption'
            fromData.id = this.otherid
          } else {
            url = '/adoption/renewAdoption'
          }
