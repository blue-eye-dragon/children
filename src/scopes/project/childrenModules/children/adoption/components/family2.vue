  ...
  <div>
    <div class="visitbtn"
      ...
      <ta-button type="primary"
                 class="addbtn"
                 @click="addAdoption">查看</ta-button>
    </div>
    <div>
      <ta-table bordered
                class="tables"
...
export default {
  ...
  data () {
    return {
      ...
      idedit: false,
      rules: {
        name: { validateTrigger: 'blur', rules: [{ required: true, message: '请输入姓名' }, { max: 72, message: '最大输入72个字符' }] },
  ...
  methods: {
    ...
    dateprocessing (data) {
      ...
      const arr = data.healthCondition.split(',')
      this.heathmodel = arr
