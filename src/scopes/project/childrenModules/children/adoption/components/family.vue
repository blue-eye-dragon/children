  ...
  <div class='tadrawer'>
    ...
    <ta-drawer title="家庭子女信息"
      ...
      <ta-form class="drawerFrom"
               ...
               layout="horizontal">
        <ta-form-item label="所属申请人"
                      fieldDecoratorId="childrensParentSituation"
                      :require="{message:'请选择所属申请人'}"
                      :labelCol="{ span: 6 }">
          <ta-select :disabled="idedit"
                     :getPopupContainer="setPopupContainer"
                     :placeholder="idedit?'':'请选择与申请人关系'">
            <ta-select-option :value="item.value"
                              v-for="(item,index) in childrensParentSituationList"
        ...
        <ta-form-item label="与收养人关系"
                      fieldDecoratorId="relationship"
                      :labelCol="{ span: 6 }"
                      :require="{message:'请选择与收养人关系'}">
          <ta-select :disabled="idedit"
                     :getPopupContainer="setPopupContainer"
                     :placeholder="idedit?'':'请选择与收养人关系'">
            <ta-select-option :value="item.value"
        ...
        <ta-form-item label="证件类型"
                      ...
                      :labelCol="{ span: 6 }"
                      :fieldDecoratorOptions="this.verificationRules('证件类型')"
        ...
        <ta-form-item label="证件号码"
                      ...
                      :labelCol="{ span: 6 }"
                      :fieldDecoratorOptions="idCardTypeToFamily == '01'?rules.idCard:this.verificationRules('证件号码', 18)">
          <ta-input :placeholder="idedit?'':'请输入证件号码'"
                    :disabled="idedit"
                    @blur="idCardValid" />
        </ta-form-item>
        <!--
...
export default {
    ...
    return {
      ...
      rules: {
        ...
        censusRegisterDetail: {
          validateTrigger: 'change',
          rules: [
            { required: true, message: '请输入户籍所在地详细地址' },
  ...
  methods: {
    ...
    add () {
      this.addAdoption()
      this.$nextTick(() => {
        this.form.setFieldsValue({ childrensParentSituation: this.adoptionTabKey })
      })
    },
    changeIdCardTypeValue (v) {
      this.$nextTick(() => {
        this.idCardTypeToFamily = v
      })
      // this.idCardTypeToFamily = v
    ...
    dateprocessing (data) {
      ...
      this.$nextTick(() => {
        this.addressmodel = [data.censusRegisterPro || '', data.censusRegisterCity || '', data.censusRegisterCounty || '', data.censusRegisterTown || '']
        const arr = data.healthCondition ? data.healthCondition.split(',') : []
        this.heathmodel = arr
