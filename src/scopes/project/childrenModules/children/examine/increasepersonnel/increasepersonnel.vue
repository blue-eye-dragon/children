      ...
      <div>
        ...
        <ta-table bordered
          ...
          <div slot="action"
               slot-scope="a,b,c"
               class="btnlist">
            <span :class="b.status=='4' ?'com_textedit':'com_textsee'"
...
export default {
  ...
  data () {
    return {
      ...
      mixListOptions: {
        ...
        mixGetDataListURL: '/addChildApi/pageCheckChild' // 数据列表接口，API地址
      },
      // 默认属性
      mixDataList: [], // 数据列表
      // f分页组件属性
  ...
  methods: {
    ...
    addAdoption (e) {
      ...
      this.$router.push({
        name: 'ipApproval',
        query: { childEnterId: e.id, workflowId: e.workflowId, workflowPointId: e.workflowPointId, flag: flag, isAnotherPlaceApply: e.isAnotherPlaceApply, isOverAge: e.isBulu == '1' }
      })
    },
    gobatch () {
      let tag = true
    ...
    childDetail (e) {
      ...
      _self.$router.push({
        ...
        query: { url: '/childInfoApi/getOrphansDisabledChildrenById', params: JSON.stringify({ id: e.id }), operateFlag: 'look_now', isOverAge: e.isBulu == '1' }
      })
    }
  }
}
