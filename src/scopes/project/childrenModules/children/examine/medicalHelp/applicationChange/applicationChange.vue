<template>
  <div class="searchdata tadrawer">
      ...
      <div class="contentin">
        ...
        <ta-table class="tables table-min-width"
          ...
          <div slot="action"
               slot-scope="text,obj,index"
               class="btnlist">
            <span @click="addAdoption(text,obj,index)"
                  :class="obj.nodesState=='2' ? 'com_audit' : 'com_textsee'">
              {{obj.nodesState=='2' ? '审批' : '查看'}}
            </span>
          </div>
        </ta-table>
      </div>
    ...
    <!-- <SalvageApprovalBatches :visible.sync="visible" msg="增员申请信息" types="1"></SalvageApprovalBatches> -->
  </div>
</template>
<script>
...
const tableColumns = [
  ...
  },
  {
    ...
    scopedSlots: { customRender: 'nodesState' }
  },
  {
    title: '操作',
  ...
  data () {
    return {
      ...
      mixListOptions: {
        ...
        mixGetDataListURL: '/medicalRescueApplyChangeAudit/list' // 数据列表接口，API地址
      },
      isdisabled: true,
      chosedata: [],
      visible: false,
