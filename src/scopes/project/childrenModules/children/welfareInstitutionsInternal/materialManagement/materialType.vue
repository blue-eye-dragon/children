...
export default {
  ...
  data () {
    return {
      ...
      mixListOptions: {
        ...
        mixGetDataListURL: '/welfareMaterialType/list', // 数据列表接口，API地址
        mixGetDataListIsPage: true, // 数据列表接口，是否需要分页？
        mixScreen: true, // 是否需要筛选
        mixDeleteURL: '/welfareMaterialType/updateDelStateById', // 删除接口，API地址
        mixExportURL: '/welfareMaterialType/export', // 导出接口，API地址
        mixAddURL: '/welfareMaterialType/save', // 新增接口
        mixEditURL: '/welfareMaterialType/update', // 编辑接口
  ...
  methods: {
    loadData () {
      ...
      this.current = 1
      this.pageSize = 10
      this.$set(this.dataObj, 'pageNumber', 1)
      this.$set(this.dataObj, 'pageSize', 10)
      // this.dataObj.pageSize = 10
    },
    setContainer () {
    ...
    async deltrueMy (text, record) {
      ...
      if (keys[0] == '0') {
        ...
        _self.$message.error(data.data[1])
        _self.fnQueryInfo()
      } else if (keys[0] == '2') {
    ...
    loadChild (expanded, record) {
      ...
      if (expanded) {
        console.log(record)
        // 对点击展开的节点请求子数据
        this.post('/welfareMaterialType/queryRecursionById', {
          id: record.id
        }).then(res => {
