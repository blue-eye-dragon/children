    ...
    <ta-col :span="24">
      <ta-form-item label="材料上传"
        ...
        <ta-upload :withCredentials="true"
                   ...
                   :multiple="true"
                   :action="action"
                   :fileList="credentialAppList"
                   :beforeUpload="beforeUpload"
                   @preview="preview"
                   @remove="handleRemove('credentialAppList',$event)"
                   @change="handleChange">
  ...
  methods: {
    ...
    handleChange (info) {
      if (info.file.status === 'uploading') {
        ...
        this.$emit('update:credentialAppList', info.fileList)
      }
      ...
      if (info.file.status === 'done') {
        if (info.file.response.errors && info.file.response.errors.length != 0) {
          ...
          this.credentialAppList.splice(-1, 1)
          return false
        }
        this.credentialAppList = info.fileList
        // 上传成功
        this.$emit('update:credentialAppList', info.fileList)
