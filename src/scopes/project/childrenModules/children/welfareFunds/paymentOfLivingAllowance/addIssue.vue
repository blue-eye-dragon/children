            ...
            <ta-row class="fromcom">
              ...
              <ta-col :span="8">
                <ta-form-item label="补发放金额"
                              ...
                              :fieldDecoratorOptions="rules.costOfLiving">
                  <ta-input-number :min="1"
                                   :max="999999"
                                   placeholder="请输入补发标准"
                                   :step="0.01"
  ...
  data () {
    return {
      ...
      rules: {
        ...
        unsupportedChildren: this.verificationRules('发放金额', true),
        costOfLiving: this.verificationRules('补发金额', true),
  ...
  methods: {
    ...
    changecostOfLiving ({ newData, record, columnKey }) {
      ...
      record.costOfLivingTotal = (
        (parseFloat(record.costOfLiving || 0)) + (parseFloat(record.replacementAmount || 0))
      ).toFixed(2)
    },
    ...
    async loadMoney (val, time) {
      ...
      if (_self.nowkey == '1') {
        ...
        _self.lifeaddFrom.setFieldsValue({ issuingType: null, costOfLiving: null, replacementYears: null, replacementYears1: null })
      }
        ...
        if (res.serviceSuccess) {
          ...
          if (data) {
            ...
            _self[_self.nowkey == '1' ? 'lifeFrom' : 'lifeaddFrom'].setFieldsValue({
              ...
              [_self.nowkey == '1' ? 'unsupportedChildren' : 'costOfLiving']: data.costOfLiving
            })
          } else {
            _self.type = '2'
            _self.type1 = '2'
    ...
    dataProcessing () {
      ...
      this.replacementList.map(item => {
        item.replacementAmount = lifeaddFromData.costOfLiving || 0
      ...
      if (this.nowkey == '2') {
        paramsFile = {
          ...
          replacementAmount: lifeaddFromData.costOfLiving,
