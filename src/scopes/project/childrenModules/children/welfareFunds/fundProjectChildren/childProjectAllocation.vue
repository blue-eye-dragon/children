...
export default {
  ...
  data () {
    return {
      ...
      mixListOptions: {
        ...
        mixGetDataListURL: '/childWelfareFundAllocate/list', // 数据列表接口，API地址
        mixAddURL: '', // 数据新增接口，API地址
        mixDeleteURL: '/childWelfareFundAllocate/updateDelStateById', // 数据删除接口，API地址
  ...
  methods: {
    routeWatchMange () {
      let baseType = this.Base.getNowPageParam().type
      this.type = baseType
      console.log(this.type)
      if (baseType == 'difficulty') {
        // 困境儿童福利补贴资金管理----困境儿童福利补贴下拨
        this.$set(
          this.mixListOptions,
          'mixGetDataListURL',
          '/troubledChildAllocate/list'
        )
        this.mixListOptions.mixDeleteURL =
          '/troubledChildAllocate/updateDelStateById' // 数据删除接口，API地址
      } else if (baseType == 'orphan') {
        // 孤儿福利补贴资金管理----孤儿基本生活费下拨
        this.$set(
          this.mixListOptions,
          'mixGetDataListURL',
          '/orphanLivingExpensesAllocate/list'
        )
        this.mixListOptions.mixDeleteURL =
          '/orphanLivingExpensesAllocate/updateDelStateById' // 数据删除接口，API地址
      } else if (baseType == 'project') {
        // 儿童福利项目资金管理----儿童福利资助下拨
        this.mixListOptions.mixGetDataListURL =
          '/childWelfareFundAllocate/list' // 数据列表接口，API地址
        this.mixListOptions.mixDeleteURL =
          '/childWelfareFundAllocate/updateDelStateById' // 数据删除接口，API地址
      }
      this.handleSubmit()
    },
    // 新增
