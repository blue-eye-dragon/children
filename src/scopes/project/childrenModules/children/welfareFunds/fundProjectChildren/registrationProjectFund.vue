...
export default {
  ...
  data () {
    return {
      ...
      mixListOptions: {
        ...
        mixGetDataListURL: '/childWelfareFundAllocate/queryPageOrgList', // 数据列表接口，API地址
  ...
  activated () {
    ...
    if (goTypeTemp == 'registrationFund-edit') {
      ...
      this.goType = '1'
      this.arrData = data.childWelfareFundAllocateMoneyVoList
      this.isok = true
  ...
  methods: {
    ...
    nextPre () {
      if (this.goType == '1') {
        ...
        objs.childWelfareFundAllocateMoneyVoList = this.obj
