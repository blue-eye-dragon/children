    ...
    <div class="contin">
      ...
      <div class="mars">
        ...
        <ta-table bordered
          ...
          <!-- <div slot="allocateMoney" slot-scope="text, record">
            <ta-table-edit v-if="disabled==false" type="inputNumber" @change="changeData"></ta-table-edit>
            <span v-if="disabled==true">{{record.allocateMoney}}</span>
          </div> -->
          <span slot="num"
                slot-scope="a, b, c">
...
export default {
  ...
  data () {
    return {
      ...
      mixListOptions: {
        ...
        mixAddURL: '/childWelfareFundAllocate/save', // 数据新增接口，API地址
        mixDeleteURL: '', // 数据删除接口，API地址
        mixEditURL: '/childWelfareFundAllocate/update' // 数据编辑接口，API地址
      },
      addTableColumns,
      addTableData: [],
  ...
  activated () {
    ...
    console.log(goTypeTemp == 'registrationFund-edit')
    if (goTypeTemp == 'registrationFundTwo-add') {
      ...
      // 编辑
      this.goType = '2'
      // let data = this.$route.params.obj
  ...
  methods: {
    ...
    getDetails (data) {
      ...
      // let sum = 0
      // this.addTableData = data.childWelfareFundAllocateMoneyVoList
      let dataTab = data.childWelfareFundAllocateMoneyVoList
      if (this.goType == '2') {
    ...
    async submit (e) {
      ...
      if (isok) {
        ...
        troubledChildAllocateVo.childWelfareFundAllocateMoneyVoList = this.addTableData
