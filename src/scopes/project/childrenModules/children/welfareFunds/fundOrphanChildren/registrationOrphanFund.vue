...
export default {
  ...
  data () {
    return {
      ...
      mixListOptions: {
        ...
        mixGetDataListURL: '/orphanLivingExpensesAllocate/queryPageOrgList', // 数据列表接口，API地址
  ...
  activated () {
    ...
    if (goTypeTemp == 'registrationFund-edit') {
      ...
      this.goType = '1'
      this.arrData = data.orphanLivingExpensesAllocateMoneyVoList
  ...
  methods: {
    ...
    nextPre () {
      if (this.goType == '1') {
        ...
        objs.orphanLivingExpensesAllocateMoneyVoList = this.obj
        ...
        this.tableData.map((item, index) => {
          this.arrData.map((items, i) => {
            if (item.orgId == items.orgId) {
              console.log(items)
              arr.push(index)
              this.tableData[index].allocateMoney = this.arrData[i].allocateMoney
              this.obj.push(items)
            }
          })
        })
