          ...
          <div class="searchcont"
            ...
            <ta-form layout="inline"
              ...
              <ta-form-item label="机构性质"
                            :labelCol="{span:7}"
                            :wrapperCol="{span:17}"
                            fieldDecoratorId="orgNatureList"
                            class="spilwid1"
                            :initValue="[]">
...
export default {
  ...
  data () {
    return {
      ...
      areaCode: '',
      mixListOptions: {
        mixGetDataListURL: '/childProtectOrgManagement/queryProtectOrgCount',
        mixGetDataListURL2: 'childProtectOrgManagement/queryProtectOrg',
  ...
  methods: {
    ...
    async getDictionarieMore () {
      ...
      this.jigoutypeList = await this.getDictionaries('TYPEOFPROTECTION')
      this.jigouorgnatureList = await this.getDictionaries('ORGNATURE')
    },
    // 健康狀況字典
    async getnation () {
      let formdata = {
    ...
    Excel () {
      ...
      let url = '/protectChildOrg/exportProtectChildOrg'
      if (this.nowkey1 == '2') { // 监护不当
        url = '/protectChildOrg/exportProtectChildOrg'
      }
