...
export default {
  ...
  updated () {
    ...
    this.setLabelAndWrapperWidth('10%', '90%', 'alonerow1') // 单独一行字段宽度控制
    this.setLabelAndWrapperWidth('14%', '86%', 'alonerow2') // 单独一行字段宽度控制
    this.setLabelAndWrapperWidth('16%', '84%', 'alonerow3') // 单独一行字段宽度控制
  },
  mounted () {
  ...
  activated () {
    if (this.$route.query.areaCode) {
      this.isshow = false
      this.isgoback = true
      // this.nowkey1 = this.$route.query.registType
  ...
  methods: {
    ...
    userPageParams () {
      ...
      formData.type = '1'
      // welfareOrgId  1000000000000215169
      ...
      formData.welfareOrgId = window.parent.indexTool.getUserInfo().orgId
      // 患艾滋病情况其他及最可能感染其他处理
    ...
    Excel () {
      ...
      const url = '/orphanLivingExpenses/exportUnattendedChildrenOnly'
