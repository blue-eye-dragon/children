  ...
  <div class="searchdata tadrawer">
        ...
        <ta-table class="tables table-min-width"
          ...
          <div slot="operate"
            ...
            <span class="com_audit"
                  v-if="b.checkStatus=='1'"
                  @click="xf(b)">审批</span>
            <span class="com_textsee"
                  v-if="b.checkStatus!=1"
                  @click="xf(b,'see')">查看</span>
          </div>
        </ta-table>
      </div>
      <div slot="footer">
    ...
    <div>
      <ta-drawer title="续发"
        ...
        <div slot="footer">
          <ta-button key="back"
                     @click="handleCancel">{{disabled?'返回':'取消'}}</ta-button>
...
export default {
  ...
  data () {
    return {
      ...
      mixListOptions: {
        ...
        mixGetDataListURL: '/overAgeFactChildApi/pageAuditOverAgeUnaChild' // 数据列表接口，API地址
      },
      // 默认属性
      mixDataList: [], // 数据列表
      // f分页组件属性
  ...
  methods: {
    ...
    submit (val) {
      const _self = this
      if (val == '0') {
        this.post('/overAgeFactChildApi/rejectOverAgeFactChild', { id: this.id }).then((res) => {
          if (res.data.data) {
            _self.$message.success('数据操作成功！')
            _self.handleCancel()
          }
      ...
      if (val == '1') {
        this.post('/overAgeFactChildApi/agreeOverAgeFactChild', { id: this.id }).then((res) => {
          if (res.data.data) {
            _self.$message.success('数据操作成功！')
            _self.handleCancel()
          }
