        ...
        <ta-table class="tables table-min-width"
          ...
          <span slot="operate"
            ...
            <span class="com_continue"
                  v-if="(isTown || isWelfare)&&!record.superventionOverDate&&record.isEnjoyMoney == '1'&&isNextState"
                  @click="xf(record)"> 续发 </span>
            <span v-if="!(isTown || isWelfare)"
                  class="com_textsee"
                  @click="see(record)"> 查看 </span>
            <span v-if="!(isTown || isWelfare)&&(isNextState || record.overAgeType == '4')"
                  class="com_textedit"
                  @click="edit(record)"> 修改 </span>
          </span>
          <div slot="superventionOverDate"
               slot-scope="val, obj">
            <span>{{val?val.substring(0, 7):'--'}}</span>
          </div>
          <template slot="name"
                    slot-scope="a,b,c">
        ...
        <ta-form :autoFormCreate="(modelForm)=>{this.modelForm = modelForm}"
          ...
          <ta-col :span="24">
            <ta-form-item label="续发截止日期"
              ...
              <ta-month-picker :disabledDate="disabledSupDate"
                               :disabled='disabled'
                               style="width: 100%" />
            </ta-form-item>
          </ta-col>
...
export default {
  ...
  data () {
    return {
      ...
      mixListOptions: {
        ...
        mixGetDataListURL: '/overAgeFactChildApi/pageOverAgeFactChild' // 数据列表接口，API地址
      },
      // 默认属性
      mixDataList: [], // 数据列表
      // f分页组件属性
  ...
  methods: {
    async edit (record, type) {
      ...
      const res1 = await this.post('/overAgeFactChildApi/beforeCheckOfOverAgeUna', { id: record.addUnattendedId })
      if (res1.data.data) {
        this.visible = true
        const res = await this.post('/overAgeFactChildApi/queryById', { addUnattendedId: record.addUnattendedId })
        if (res.serviceSuccess || type == 'see') {
          const { data } = res.data
          this.id = data.id
    ...
    xf (record, type) {
      this.post('/overAgeFactChildApi/beforeCheckOfOverAgeUna', { id: record.addUnattendedId }).then((res1) => {
        if (res1.data.data) {
          this.radio = ''
          this.visible = true
          this.disabled = false
    ...
    jy (text, obj, index) {
      ...
      this.post('/overAgeFactChildApi/beforeCheckOfOverAgeUna', { id: obj.addUnattendedId }).then((res1) => {
        if (res1.data.data) {
