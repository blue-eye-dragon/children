      ...
      <div>
        ...
        <ta-table bordered
          ...
          <div slot="action"
               slot-scope="a,b,c"
               class="btnlist">
            <span :class="b.status=='4' ? 'com_audit' : 'com_textsee'"
                  @click="addAdoption(b)">{{b.status=='4' ? '审批' : '查看'}}</span>
          </div>
        </ta-table>
      </div>
      <div slot="footer">
...
<script>
import ApprovalBatches from './components/secondaryApprovalGo'
...
export default {
  ...
  data () {
    return {
      ...
      sexList: [], // 性别字典数据
      types: '2', // 类别（入院1，离院2，变更3）

      // 设置属性
      mixListOptions: {
        mixActivatedIsNeed: true, // 此页面是否在激活（进入）时，调用查询数据列表接口？
        mixGetDataListURL: '/overAgeTipApi/pageChildSupervention' // 数据列表接口，API地址
      },
      // f分页组件属性
  ...
  methods: {
    ...
    async getMenuDatas () {
      ...
      this.childrenTypeList = await this.getDictionaries(childTypeDict)
      this.sexList = await this.getDictionaries('SEX') // 获取性别字典数据
    },
    // 导出
    async exportFileList () {
      var data = this.userPageParams()
      this.exportFile('/reductionChild/exportReductionChildCheck', data)
    },
    deletes (val) {
    ...
    addAdoption (e) {
      ...
      this.$router.push({
        ...
        query: { id: e.id, childEnterId: e.childEnterId, workflowId: e.workflowId, flag: flag, workflowPointId: e.workflowPointId }
      })
    },
    gobatch () {
      let tag = true
