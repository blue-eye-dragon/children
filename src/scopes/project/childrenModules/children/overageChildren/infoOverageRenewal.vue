...
export default {
  ...
  data () {
    return {
      ...
      mixListOptions: {
        ...
        mixGetDataListURL: '/overAgeFactChildApi/pageFactOverAgeSupervention' // 数据列表接口，API地址
      },
      // 默认属性
      mixDataList: [], // 数据列表
      // f分页组件属性
  ...
  methods: {
    ...
    look (e) {
      this.visible = true
      this.post('/overAgeFactChildApi/detailFactOverAgeSupervention', { id: e.id }).then((res) => {
        if (res.serviceSuccess) {
          let { data } = res.data
          ...
          data['superventionOverDate'] = data.superventionOverDate ? moment(data.superventionOverDate) : null
