<template>
  <div class="nofy">

    <div class="titletop">

      <img :src="imgsrc"
           alt />

      <span>{{titletext}}</span>
      <ta-checkbox-group @change="ismanChange"
                         v-model="copyisman"
                         class="radio_grops"
                         :disabled='disabled'>
        <ta-checkbox :value="types"
                     class="apply">单亲</ta-checkbox>
      </ta-checkbox-group>

    </div>

    <ta-col :span='24'>

      <ta-col :span="12">

        <ta-form-item label="信息是否完整"
                      fieldDecoratorId="infoCase"
                      :labelCol="{ span: 12 }"
                      :wrapperCol="{ span: 12 }"
                      initValue='1'
                      :require="isrequired ? {message:'请选择信息是否完整'} : false">

          <ta-radio-group :disabled="isDisabledForm"
                          @change="mustController">

            <ta-radio :value="item.value"
                      v-for="(item,index) in [{value:'1',label:'是'},{value:'0',label:'否'}]"
                      :key="index">{{item.label}}</ta-radio>

          </ta-radio-group>

        </ta-form-item>

      </ta-col>

      <ta-col :span="12">

        <ta-form-item label="证明材料"
                      fieldDecoratorId="uploadFileT"
                      :labelCol="{ span: 10 }"
                      :wrapperCol="{ span: 14 }"
                      v-if="!mustTip"
                      :require="{message:'请上传证明材料'}">

          <ta-upload :withCredentials="true"
                     name="file"
                     :data="data_"
                     :multiple="true"
                     :action="action"
                     :fileList="parentlist"
                     @change="handleChange"
                     @preview="confirmDownload"
                     :beforeUpload="beforeUpload">

            <ta-button v-if="!isDisabledForm">

              <ta-icon type="upload" />上传文件

            </ta-button>

          </ta-upload>

        </ta-form-item>

      </ta-col>

    </ta-col>
    <!-- <ta-col :span='24'>
      <ta-col :span="12">
        <ta-form-item label="是否单亲"
                      :labelCol="{ span: 10}"
                      :wrapperCol="{ span: 14 }"
                      fieldDecoratorId="uniparental"
                      :initValue="'0'"
                      :require="issure?{message:'请选择是否单亲'}:''">
          <ta-radio-group @change="changedanqin"
                          :disabled="isDisabledForm">
            <ta-radio value="0">否</ta-radio>
            <ta-radio value="1">是</ta-radio>
          </ta-radio-group>
        </ta-form-item>
      </ta-col>
      <ta-col :span="12" v-if="isdanqin">
        <ta-form-item label="单亲原因"
                      :labelCol="{ span: 7 }"
                      :wrapperCol="{ span: 17 }"
                      fieldDecoratorId="singleParentReason"
                      :require="issure?{message:'请选择单亲原因'}:''">
          <ta-radio-group @change="danqinyuanyin"
                          :disabled="isDisabledForm">

          <ta-radio :value="item.value"
                    v-for="(item,index) in getsingleList"
                    :key="index">{{item.label}}</ta-radio>
          </ta-radio-group>
        </ta-form-item>
      </ta-col>
    </ta-col> -->
    <!-- <ta-col :span="24" v-if="isdqrs">
        <ta-form-item label="其他原因"
                      :labelCol="{ span: 5 }"
                      :wrapperCol="{ span: 19 }"
                      fieldDecoratorId="singleParentReasonOther"
                      :fieldDecoratorOptions="this.verificationRules('其他原因',50)">
          <ta-input :placeholder="'请输入其他原因'"
                    maxlength='72'/>
        </ta-form-item>
    </ta-col>  -->
    <ta-col :span='24'>
      <ta-form-item label="特殊情况"
                    :labelCol="{ span: 4 }"
                    :wrapperCol="{ span: 20 }"
                    fieldDecoratorId="otherSpecialCircumstances">
        <ta-radio-group @change="zkChange"
                        :disabled="isDisabledForm">
          <ta-radio :value="item.value"
                    @click="doSum(item.value, 'otherSpecialCircumstances')"
                    v-for="(item,index) in presentsituationlist"
                    :key="index">{{item.label}}</ta-radio>

        </ta-radio-group>

      </ta-form-item>
    </ta-col>
    <ta-col :span="24">
      <ta-form-item label="其他"
                    v-if="zkval==9"
                    fieldDecoratorId="otherSpecialCircumstancesOther"
                    :fieldDecoratorOptions="this.verificationRules('其他',50)">

        <!-- :fieldDecoratorOptions="isrequired?rules.currentSituationOther:rules.currentSituationOther2" -->

        <ta-input :disabled="isDisabledForm"
                  maxlength='50'
                  :placeholder="isDisabledForm?'':'请输入其他情况'" />

      </ta-form-item>
      <!-- <ta-form-item label="死亡日期"
                    v-if="zkval==1"
                    :labelCol="{ span: 5 }"
                    :wrapperCol="{ span: 19 }"
                    fieldDecoratorId="dieDate"
                    :require="{message:'请选择死亡日期'}">
        <ta-date-picker style="width: 50%;"
                        :disabled='isDisabledForm'
                        :placeholder="isDisabledForm?'':'请选择死亡日期'" />
      </ta-form-item> -->
    </ta-col>
    <ta-col :span="12">

      <ta-form-item label="姓名"
                    fieldDecoratorId="name"
                    :labelCol="{ span: 9 }"
                    :wrapperCol="{ span: 15 }"
                    :fieldDecoratorOptions="mustTip && isrequired?rules.name:rules.name2">

        <ta-input :placeholder="isDisabledForm?'':'请输入姓名'"
                  maxlength='72'
                  :disabled="isDisabledForm" />

      </ta-form-item>

      <ta-form-item label="证件号码"
                    fieldDecoratorId="idCard"
                    :labelCol="{ span: 9 }"
                    :wrapperCol="{ span: 15 }"
                    v-if="mustTip!='1' || !islive"
                    :fieldDecoratorOptions="(cardType=='01' && isrequired)?rules.idCard2:this.verificationRulesNotMustFill('证件号码',18)">

        <ta-input :placeholder="isDisabledForm?'':'请输入证件号码'"
                  maxlength='18'
                  :disabled="isDisabledForm" />

      </ta-form-item>

      <ta-form-item label="证件号码"
                    fieldDecoratorId="idCard"
                    :labelCol="{ span: 9 }"
                    :wrapperCol="{ span: 15 }"
                    v-else
                    :fieldDecoratorOptions="(cardType=='01' && isrequired)?rules.idCard:rules.idCard2">

        <ta-input :placeholder="isDisabledForm?'':'请输入证件号码'"
                  maxlength='18'
                  :disabled="isDisabledForm" />

      </ta-form-item>
    </ta-col>
    <ta-col :span="12">

      <ta-form-item label="证件类型"
                    fieldDecoratorId="memberIdType"
                    :labelCol="{ span: 9 }"
                    :wrapperCol="{ span: 15 }"
                    :fieldDecoratorOptions="(mustTip && islive && isrequired)?this.verificationRules('证件类型'):{rules: [{ required: false }]}">

        <ta-select allowClear
                   @change="changeCardType"
                   :placeholder="isDisabledForm?'':'请选择证件类型'"
                   :disabled="isDisabledForm">

          <ta-select-option v-for="(item,index) in cardTypeList"
                            :value="item.value"
                            :key="index">{{item.label}}</ta-select-option>

        </ta-select>

      </ta-form-item>

      <ta-form-item label="联系电话"
                    fieldDecoratorId="contactNumber"
                    :labelCol="{ span: 9 }"
                    :wrapperCol="{ span: 15 }"
                    :fieldDecoratorOptions="verificationRulesNotMustFill('联系电话', { type: 'phone' })">
        <ta-input :placeholder="isDisabledForm?'':textphone"
                  :disabled="isDisabledForm" />
      </ta-form-item>
    </ta-col>

    <ta-col :span='24'>
      <ta-col :span='12'>
        <ta-form-item label="现住址情况"
                      fieldDecoratorId="citizenship"
                      :labelCol="{span:9}"
                      :wrapperCol="{span:15}"
                      :fieldDecoratorOptions="{rules: [ { required: (mustTip &&isknow&& islive && isrequired), message: '请选择现住址情况' }], initialValue: ['1']} ">
          <ta-select :placeholder="isDisabledForm?'':'请选择现住址情况'"
                     allowClear
                     :disabled="isDisabledForm"
                     @change="implicate">
            <ta-select-option v-for="(item,index) in [{'label':'国内','value':'1'},{'label':'国外','value':'2'}]"
                              :value="item.value"
                              :key="index">{{item.label}}</ta-select-option>
          </ta-select>
        </ta-form-item>
      </ta-col>
    </ta-col>
    <ta-col :span='12'>
      <AddressComponent :labelwidth="labelwidth"
                        :disabled="isDisabledForm"
                        :addressmodel="addressFMModel"
                        :msg="'户籍所在地'"
                        :fileldName="'censusRegisterSD'"
                        @setaddress="setAddressFM"
                        :arealevel="'4'"
                        :clearPlaceHolder='isDisabledForm'
                        @adsvalchange="likeOut"
                        :isRequire="mustTip && islive && isrequired"></AddressComponent>

      <AddressComponent :labelwidth="labelwidth"
                        :disabled="isDisabledForm?true:citizenship"
                        :addressmodel="addressFMModelNow"
                        :msg="'现居住地址'"
                        :fileldName="'currentAddress'"
                        @setaddress="setAddressFMNow"
                        :clearPlaceHolder='isDisabledForm'
                        :arealevel="'4'"
                        @adsvalchange="likeOut"
                        :isRequire="mustTip && islive && isknow &&!citizenship && isrequired"></AddressComponent>

    </ta-col>

    <ta-col :span="12">
      <ta-col :span='17'>

        <ta-form-item label=""
                      fieldDecoratorId="censusRegisterDetail"
                      :labelCol="{ span: 0 }"
                      :wrapperCol="{ span: 24,offset: 1 }"
                      :fieldDecoratorOptions="(mustTip && islive && isrequired)?this.verificationRules('详细地址',100):this.verificationRulesNotMustFill('详细地址',100)">

          <ta-input :placeholder="isDisabledForm?'':'请输入详细地址'"
                    maxlength='100'
                    :disabled="isDisabledForm"
                    @change="likeOut" />

        </ta-form-item>

      </ta-col>

      <ta-col :span='7'>

        <ta-form-item label="同现住址"
                      fieldDecoratorId="sameAddress"
                      :fieldDecoratorOptions="{initialValue: []}"
                      :labelCol="{ span: 23 }"
                      :wrapperCol="{ span: 1 }">

          <ta-checkbox-group :options="sameaddresslist"
                             @change="likeaddress"
                             :disabled="isDisabledForm?true:citizenship" />

        </ta-form-item>

      </ta-col>

      <ta-col :span='24'>

        <ta-form-item label=""
                      fieldDecoratorId="currentAddressDetail"
                      :labelCol="{ span: 0 }"
                      :wrapperCol="{ span: 24,offset: 1 }"
                      :fieldDecoratorOptions="(mustTip && islive && isknow&&!citizenship && isrequired)?this.verificationRules('详细地址',100):this.verificationRulesNotMustFill('详细地址',100)">

          <ta-input :placeholder="isDisabledForm?'':'请输入详细地址'"
                    maxlength='100'
                    :disabled="isDisabledForm"
                    @change="likeOut" />

        </ta-form-item>

      </ta-col>

    </ta-col>

    <ta-col :span="24">

      <ta-form-item label="健康状况"
                    fieldDecoratorId="healthStatus"
                    :labelCol="{ span: 6 }"
                    :wrapperCol="{ span: 18 }"
                    :fieldDecoratorOptions="(mustTip && islive && isknow && isrequired)?this.verificationRules('健康状况'):{rules: [{ required: false }]}">

        <ta-radio-group @change="onChangeFM"
                        :disabled="isDisabledForm">

          <ta-radio :value="item.value"
                    v-for="(item,index) in thisP.heathlist"
                    :key="index">{{item.label}}</ta-radio>

        </ta-radio-group>

      </ta-form-item>

      <ta-form-item v-if="heathval==Enum.HEALTHSTATUS.OTHER"
                    label="其他"
                    fieldDecoratorId="healthStatusOther"
                    :labelCol="{ span: 6 }"
                    :wrapperCol="{ span: 18 }"
                    :fieldDecoratorOptions="this.verificationRules('其他',50)">

        <ta-input :disabled="isDisabledForm"
                  maxlength='50'
                  :placeholder="isDisabledForm?'':'请输入其他情况'" />

      </ta-form-item>

      <div class="canji"
           v-if="heathval==Enum.HEALTHSTATUS.DISABILITY">

        <ta-form-item label="残疾类型"
                      fieldDecoratorId="disabledType"
                      :labelCol="{ span: 6 }"
                      :wrapperCol="{ span:18 }"
                      :initValue="[]"
                      :require="{message:'请选择残疾类型'}">

          <ta-checkbox-group :options="disabiliytypelist"
                             @change="setMulti"
                             :disabled="isDisabledForm"
                             :values="disabilityTypeValues" />

        </ta-form-item>

        <ta-form-item label="残疾等级"
                      fieldDecoratorId="disabledLevel"
                      :labelCol="{ span: 6 }"
                      :wrapperCol="{ span: 18 }">

          <ta-radio-group :disabled="isDisabledForm">

            <ta-radio :value="item.value"
                      v-for="(item,index) in thisP.canjileave"
                      :key="index">{{item.label}}</ta-radio>

          </ta-radio-group>

          <ta-radio-group style="float:left;"></ta-radio-group>

        </ta-form-item>

      </div>

      <!--重病-->

      <div class="canji"
           v-if="heathval==Enum.HEALTHSTATUS.SERIOUSILLNESS">

        <ta-form-item label="重病"
                      fieldDecoratorId="disabledType"
                      :labelCol="{ span: 6 }"
                      :wrapperCol="{ span: 18 }"
                      :initValue="[]"
                      :require="{message:'请选择重病!'}">

          <ta-checkbox-group :options="disabiliytypelist"
                             :disabled="isDisabledForm"
                             @change="seriousillnesschange" />

        </ta-form-item>

        <ta-form-item v-if="serioushvalserioushval"
                      label="其他"
                      fieldDecoratorId="healthStatusOther"
                      :labelCol="{ span: 6 }"
                      :wrapperCol="{ span: 18 }"
                      :fieldDecoratorOptions="this.verificationRules('其他',50)">

          <ta-input maxlength='50'
                    :disabled="isDisabledForm"
                    :placeholder="isDisabledForm?'':'请输入其他情况'" />

        </ta-form-item>

      </div>

      <!--慢性病-->

      <div class="canji"
           v-if="heathval==Enum.HEALTHSTATUS.CHRONICDISEASE">

        <ta-form-item label="慢性病"
                      fieldDecoratorId="disabledType"
                      :labelCol="{ span: 6 }"
                      :wrapperCol="{ span: 18 }"
                      :initValue="[]"
                      :require="{message:'请选择慢性病!'}">

          <ta-checkbox-group :options="disabiliytypelist"
                             :disabled="isDisabledForm"
                             @change="seriousillnesschange" />

        </ta-form-item>

        <ta-form-item v-if="serioushvalserioushval"
                      label="其他"
                      fieldDecoratorId="healthStatusOther"
                      :labelCol="{ span: 6 }"
                      :wrapperCol="{ span: 18 }"
                      :fieldDecoratorOptions="this.verificationRules('其他',50)">

          <ta-input :disabled="isDisabledForm"
                    maxlength='50'
                    :placeholder="isDisabledForm?'':'请输入其他情况'" />

        </ta-form-item>

      </div>

      <!-- <ta-form-item

                      v-if="familyBreadwinners"

                      label="家庭经济来源"

                      fieldDecoratorId="familyBreadwinner"

                      :labelCol="{ span: 6 }"

                      :wrapperCol="{ span: 18 }"

                      :initValue="[]"

                      :require="{message:'家庭经济来源'}"

                    >

                      <ta-checkbox-group :options="familyComeList" :disabled="isDisabledForm" @change="familyCome" />

                      </ta-form-item> v-else-->

      <ta-form-item label="家庭经济来源"
                    fieldDecoratorId="familyBreadwinner"
                    :labelCol="{ span: 6 }"
                    :wrapperCol="{ span: 18 }"
                    :initValue="[]"
                    v-if="mustTip"
                    :require="(islive && isknow && isrequired)?{message:'请选择家庭经济来源'}:false">

        <ta-checkbox-group :options="familyComeList"
                           :disabled="isDisabledForm"
                           @change="familyCome" />

      </ta-form-item>

      <ta-form-item label="家庭经济来源"
                    fieldDecoratorId="familyBreadwinner"
                    :labelCol="{ span: 6 }"
                    :wrapperCol="{ span: 18 }"
                    :initValue="[]"
                    v-else>

        <ta-checkbox-group :options="familyComeList"
                           :disabled="isDisabledForm"
                           @change="familyCome" />

      </ta-form-item>

      <ta-form-item v-if="familyBreadwinners"
                    label="其他"
                    fieldDecoratorId="familyBreadwinnerOther"
                    :labelCol="{ span: 6 }"
                    :wrapperCol="{ span: 18 }"
                    :fieldDecoratorOptions="this.verificationRules('其他',50)">

        <ta-input :disabled="isDisabledForm"
                  maxlength='50'
                  :placeholder="isDisabledForm?'':'请输入其他情况'" />

      </ta-form-item>
    </ta-col>

    <!-- </ta-col> -->

  </div>
</template>
<script>
import AddressComponent from '@/scopes/project/common/components/addressComponent'
export default {
  components: {
    AddressComponent
  },
  props: {
    citizenship: {
      type: Boolean,
      required: true
    },
    issure: {
      type: Boolean,
      required: true
    },
    getsingleList: {
      type: Array,
      required: true
    },
    isjianhu: {
      type: Boolean,
      required: true
    },
    isjiedu: {
      type: Boolean,
      required: true
    },
    isknow: {
      type: Boolean,
      required: true
    },
    islive: {
      type: Boolean,
      required: true
    },
    isdanqin: {
      type: Boolean,
      required: true
    },
    isdqrs: {
      type: Boolean,
      required: true
    },
    types: {
      type: String,
      required: true
    },
    zkval: {
      type: String,
      required: true
    },
    familyBreadwinners: {
      type: Boolean,
      required: true
    },
    disabled: {
      type: Boolean,
      required: true
    },
    isman: {
      type: Array,
      required: true
    },
    isrequired: {
      type: Boolean,
      required: true
    },
    id: { // 进入编辑时的id
      type: String,
      required: false
    },
    oldIdCard: { // 进入编辑时的身份证号
      type: String,
      required: false
    },
    thisP: {
      required: true
    },
    heathval: {
      type: String,
      required: true,
      default: '0'
    },
    serioushval: {
      type: Boolean,
      required: true
    },
    disabiliytypelist: {
      type: Array,
      required: true
    },
    addressFMModelNow: {
      type: Array,
      required: true
    },
    addressFMModel: {
      type: Array,
      required: true
    },
    mustTip: {
      type: Boolean,
      required: true
    },
    parentlist: {
      type: Array,
      required: true
    },
    data_: {
      type: Object,
      required: true
    },
    // 单亲禁用表单
    isSingleDisabled: {
      type: Boolean,
      default: false
    }
  },
  data () {
    return {
      disabilityTypeValues: [],
      // isrequired: false,
      labelwidth: {
        label: 9,
        wrapper: 15
      },
      sameaddresslist: [{ label: '', value: '1' }],

      titletext: '',
      imgsrc: '',
      imgsrc1: require('./images/parentman.png'),
      imgsrc2: require('./images/parentwomen.png'),
      // zkother: '',
      presentsituationlist: [],
      rules: {
        name: this.verificationRules('姓名', 72),
        name2: this.verificationRulesNotMustFill('姓名', 72),
        idCard: {
          validateTrigger: 'blur',
          normalize: (e) => { return e && e.replace(/\x/g, 'X') },
          rules: [
            { required: true, message: '请输入身份证号' },
            { validator: this.idcardrole }
          ]
        },
        idCard2: this.verificationRulesNotMustFill('证件号码'),
        // healthStatus: this.verificationRulesNotMustFill('健康状况',),
        contactNumber: this.phoneRule(false),
        contactNumber2: this.phoneRule(true),
        currentSituation: this.verificationRules('其他特殊情况'),
        currentSituation2: {
          validateTrigger: 'change',
          rules: [{ required: false }]
        },
        currentSituationOther: this.verificationRules('其他情况', 100),
        currentSituationOther2: this.verificationRulesNotMustFill(
          '其他情况',
          100
        )
      },
      copyisman: [],
      familyComeList: [],
      cardTypeList: [],
      serioushvalserioushval: false,
      cardType: '01',
      judge: false,
      textphone: '请输入联系电话',
      faData: null,
      moData: null
    }
  },
  activated () {
    this.cardType = '01'
    this.faData = null
    this.moData = null
  },
  mounted () {
    // this.isrequired = false
    this.getpresentsituation()

    this.serioushvalserioushval = this.serioushval
    this.types == 1 ? (this.titletext = '父亲') : (this.titletext = '母亲')
    this.types == 1
      ? (this.imgsrc = this.imgsrc1)
      : (this.imgsrc = this.imgsrc2)
  },
  computed: {
    action () {
      return window.faceConfig.basePath + '/file/uploadFile'
    },
    // 禁用表单
    isDisabledForm () {
      return this.disabled || this.isSingleDisabled || !this.isrequired
    }
  },
  methods: {
    async idcardrole (rule, value, callback) {
      if ((!this.id || value != this.oldIdCard) && value) { // 新增或编辑但身份证号改变时，进行验重操作
        this.idCardValidate(rule, value, callback, this, null)
        const res = await this.post('LeftOverChild/getParentInfoByIdCard', {
          idCard: value,
          parentIdCardType: this.cardType
        })
        const parentData = res.data.data
        if (parentData && parentData.length > 0) {
          parentData.forEach(item => {
            if (item.parentType == 1) {
              this.faData = item
            }
            if (item.parentType == 2) {
              this.moData = item
            }
          })
          if (this.types == 1) {
            this.$emit('setValueByParentIdCardF', this.faData)
          } else {
            this.$emit('setValueByParentIdCardM', this.moData)
          }
        }
      } else { // 否则不进行验重
        this.idCardValidate(rule, value, callback, this, null)
      }
    },
    // 现住址触发
    implicate (v) {
      this.thisP.citizenshipChange(v == '2', this.types)
    },
    textphone1 () {
      if (this.isjianhu) {
        this.textphone = '请输入监护所联系电话'
      } else if (this.isjiedu) {
        this.textphone = '请输入戒毒所联系电话'
      } else {
        this.textphone = '请输入联系电话'
      }
    },
    setMulti (e) {
      const index = e.indexOf(this.Enum.DISABILITYTYPE.DCCJ)
      // 选择超过两个（不包括多重残疾）时，勾选多重残疾
      if (index == -1 && e.length > 1) {
        e.push(this.Enum.DISABILITYTYPE.DCCJ)
        this.disabilityTypeValues = e
      }
      // 除多重残疾外，选择一个时，取消勾选多重残疾
      if (index != -1 && e.length == 2) {
        e.splice(index, 1)
        this.disabilityTypeValues = e
      }
      // 不能直接选择多重残疾
      if (index != -1 && e.length == 1) {
        e.splice(index, 1)
        this.disabilityTypeValues = e
      }
    },
    // 附件
    handleChange (info) {
      if (this.disabled) {
        return false
      }
      if (info.file.status === 'uploading') {
        this.$emit('update:parentlist', info.fileList)
      }
      if (info.file.status === 'removed') {
        if (this.disabled) {
          return false
        }
        this.$emit('update:parentlist', info.fileList)
      }
      if (info.file.status === 'done') {
        if (info.file.response.errors && info.file.response.errors.length != 0) {
          this.$message.error('上传失败')
          const arrlist = [...this.parentlist]
          arrlist.splice(-1, 1)
          this.$emit('update:parentlist', arrlist)
          return false
        }
        // 上传成功
        this.$emit('update:parentlist', info.fileList)
        this.$message.success('文件上传成功!')
      }
    },
    confirmDownload (file) {
      this.downloadFile(file)
    },
    beforeUpload (file) {
      return this.before_Upload(file)
    },
    // en

    //  @click="deuRadio(item.value,'healthStatus',types)"
    // deuRadio (v, a, b) {
    //   if (b == 1) {
    //     if (a == 'healthStatus') {
    //       if (v == this.thisP.fatherForm.getFieldsValue().healthStatus) {
    //         this.thisP.fatherForm.setFieldsValue({healthStatus: ''})
    //       }
    //     }
    //   }
    // },
    mustController (v) {
      const status = (v.target.value == '1')
      this.thisP.mustTipchange(status, this.types)
      if (this.types == 1) {
        this.thisP.heathvalF = '0'
        this.thisP.zkval = '0'
        this.thisP.familyBreadwinners1 = false
      } else {
        this.thisP.heathvalM = '0'
        this.thisP.zkval1 = '0'
        this.thisP.familyBreadwinners2 = false
      }
    },
    changeCardType (v) {
      this.cardType = v
      if (v == undefined) {
        this.cardType = 0
        if (this.types == 1) {
          this.$nextTick(() => {
            this.thisP.fatherForm.setFieldsValue({
              idCard: ''
            })
          })
        } else {
          this.$nextTick(() => {
            this.thisP.motherForm.setFieldsValue({
              idCard: ''
            })
          })
        }
      }
    },
    // 地址传值
    setAddressFMNow () {
      // this.$emit('setAddressFMNow', this.types)
      this.$nextTick(() => {
        if (this.types == 1) { this.$emit('setAddressFNow') } else { this.$emit('setAddressMNow') }
      })
    },
    setAddressFM () {
      this.$nextTick(() => {
        if (this.types == 1) { this.$emit('setAddressF') } else { this.$emit('setAddressM') }
      })
    },
    likeOut () {
      let formName = []
      if (this.types == 1) {
        formName = this.thisP.fatherForm
      } else {
        formName = this.thisP.motherForm
      }
      const nowData = formName.getFieldsValue()
      if (nowData.sameAddress == []) { } else {
        formName.setFieldsValue({
          sameAddress: []
        })
      }
    },
    likeaddress (q) {
      let formName = []

      if (this.types == 1) {
        formName = this.thisP.fatherForm
      } else {
        formName = this.thisP.motherForm
      }
      const data = formName.getFieldsValue()
      this.$nextTick(() => {
        if (q == '1') {
          let arrs = []
          if (data.censusRegisterSD != undefined) {
            arrs = [...data.censusRegisterSD]
          }
          if (this.types == 1) {
            this.thisP.addressFMModelNow1 = arrs
          } else {
            this.thisP.addressFMModelNow2 = arrs
          }
          formName.setFieldsValue({
            currentAddress: arrs
          })
          formName.setFieldsValue({
            currentAddressDetail: data.censusRegisterDetail
              ? data.censusRegisterDetail
              : ''
          })
        } else { }
      })
    },

    familyCome (v) {
      this.thisP.familyCome(v, this.types)
    },
    onChangeFM (e) {
      if (this.types === '1') {
        this.thisP.fatherForm.setFieldsValue({
          disabledType: [],
          healthStatusOther: ''
        })
      } else {
        this.thisP.motherForm.setFieldsValue({
          disabledType: [],
          healthStatusOther: ''
        })
      }
      this.thisP.serioushvals(false, this.types)
      this.thisP.heathvals(e, this.types)
      this.thisP.getdisabiliytype(e.target.value, this.types)
      this.thisP.setHeight('ktone', 'ktoneout')
    },
    seriousillnesschange (val) {
      if (
        val[val.length - 1] == this.Enum.HEALTHSTATUS.OTHERSERIOUSILLNESS ||
        val[val.length - 1] == this.Enum.HEALTHSTATUS.OTHERCHRONICDISEASE
      ) {
        this.serioushvalserioushval = true
      } else {
        this.serioushvalserioushval = false
      }
      if (val.length > 1) {
        if (
          val[val.length - 1] == this.Enum.HEALTHSTATUS.OTHERSERIOUSILLNESS ||
          val[val.length - 1] == this.Enum.HEALTHSTATUS.OTHERCHRONICDISEASE
        ) {
          const arr = []
          arr.push(val[val.length - 1])
          this.$nextTick(() => {
            if (this.types == '1') {
              this.thisP.fatherForm.setFieldsValue({
                disabledType: arr
              })
            } else {
              this.thisP.motherForm.setFieldsValue({
                disabledType: arr
              })
            }
          })
        }
        if (
          val[0] == this.Enum.HEALTHSTATUS.OTHERSERIOUSILLNESS ||
          val[0] == this.Enum.HEALTHSTATUS.OTHERCHRONICDISEASE
        ) {
          const arr = [...val]
          arr.shift()
          this.$nextTick(() => {
            if (this.types == '1') {
              this.thisP.fatherForm.setFieldsValue({
                disabledType: arr
              })
            } else {
              this.thisP.motherForm.setFieldsValue({
                disabledType: arr
              })
            }
          })
        }
      }
      this.thisP.setHeight('ktone', 'ktoneout')
    },

    ismanChange (val) {
      const arr = []
      arr.push(val[val.length - 1])
      this.$emit('update:isman', arr)
      this.$emit('ismanChange', arr)
    },
    // 字典
    async getpresentsituation () {
      this.presentsituationlist = await this.getDictionaries(
        'PRESENTSITUATION'
      )
      this.familyComeList = await this.getDictionaries('FAMILYSOURCEINCOME')
      this.cardTypeList = await this.getDictionaries('IDCARD_TYPE')
    },
    inpchanges (e) { },
    zkChange (e) {
      this.$emit('zkChange', e.target.value, this.types)
    },
    doSum (a, b) {
      this.$emit('parentRadio', a, b, this.types)
    }
  },
  watch: {
    isman (val) {
      this.copyisman = [...val]
      if (val.length == 0) {
        this.$emit('update:isrequired', true)
      } else {
        if (!val[0]) {
          this.$emit('update:isrequired', true)
        } else {
          if (this.types != val[val.length - 1]) {
            this.$emit('update:isrequired', false)
          } else {
            this.$emit('update:isrequired', true)
          }
        }
      }
    }
  }
}
</script>
<style lang="less" scoped>
.titletop {
  width: 100%;
  height: 30px;
  display: flex;
  justify-content: center;
  align-items: center;
  font-size: 16px;
  font-weight: 600;
  color: rgba(0, 0, 0, 0.85);
  margin: 10px 0 20px 0;
}

.titletop img {
  height: 100%;
  margin-right: 10px;
  margin-left: 12%;
}

.nofy /deep/.ant-radio-wrapper {
  margin-right: 11px;
}

.radio_grops {
  margin-left: 20px;
}
</style>
