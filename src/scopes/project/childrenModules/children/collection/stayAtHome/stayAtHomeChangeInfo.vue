        ...
        <div class="operateBtn">
          <div class="radioClass leftClass">
            <ta-radio-group v-model="nowkey"
              ...
              <ta-radio-button value="2">已退出列表</ta-radio-button>
              <!-- <ta-radio-button value="3">已超龄列表</ta-radio-button> -->
            </ta-radio-group>
          </div>
...
export default {
  ...
  data () {
    return {
      ...
      disabled: false,
      mixListOptions: {
        mixGetDataListURL: '/LeftOverChild/list', // '/LeftOverChild/listChange', // 数据列表接口，API地址
  ...
  methods: {
    async del (v, o) {
      const res = await this.post('/LeftOverChild/updateDelStateById', { id: o.leftOverChildId })
      if (res.errors) {
    ...
    async over (value, obj, index) {
      ...
      if (index == 'see') {
        this.orSee = true
        const res = await this.post('/LeftOverChild/queryById', { id: obj.leftOverChildId })
        const data = res.data.data.leftOverChildVo
        if (data) {
          this.$nextTick(() => {
    ...
    async reOver (value, obj, tp) {
      ...
      const chenck = await this.post('/LeftOverChild/getTroubleChildMsgByChildId', { id: obj.id })
