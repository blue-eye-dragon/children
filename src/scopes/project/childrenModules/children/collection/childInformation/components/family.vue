      ...
      <ta-table bordered
        ...
        <span slot="relationship"
          ...
          <span>
            ...
            <!-- ? CollectionLabel("RELATION", val) + "：本人" -->
        </span>
        <div slot="action"
             slot-scope="val, obj, index"
             class="btnlist">
...
export default {
  ...
  methods: {
    ...
    subfamily () {
      ...
      _self.form.validateFields((err, values) => {
        if (_self.isOneself == 1) {
          // 是本人
          if (err) {
            if (err.name || err.birthday || err.adisCondition) {
              this.$message.error('请先完善儿童基本信息内容！')
            }
            isok = false
          }
        } else {
      ...
      if (_self.valueSelect == _self.Enum.RELATION.QT) {
        let errmsg = _self.setErrorMessage(_self.valueInput)
        if (errmsg) {
          isok = false
        }
      }
    ...
    edit (val, obj, index) {
      ...
      if (objs.currentSituation == _self.Enum.CURRENTSITUATION.SW) { // 死亡
        _self.isDeath = true
      } else { // 非死亡
        _self.isDeath = false
      }
