        ...
        <ta-tab-pane tab="儿童基本信息"
          ...
          <div ref="oneout"
            ...
            <ta-form ref="one"
              ...
              <poorFamiliesChildrenBaseInfo :nowkeys='nowkeys'
                                            :familyPoorOthers="familyPoorOther"
                                            ...
                                            @setHeight="setHeight('one', 'oneout')"></poorFamiliesChildrenBaseInfo>
            ...
            <ta-form ref="three"
              ...
              <ta-row class="fromcom">
                ...
                <ta-col :span="24">
                  ...
                  <div v-if="xyval==Enum.FAMILYSOURCEINCOME.QT">
                ...
                <ta-col :span='12'>
                  ...
                  <ta-form-item label="救助起始日期"
                    ...
                    <ta-date-picker style="width: 100%;"
                                    format="YYYY-MM-DD"
                                    @change="onChangeTime"
                                    :placeholder="disabled?'':'请选择救助起始日期'"
                                    :disabled="disabled"
                ...
                <ta-col :span='12'>
                  ...
                  <ta-form-item label="救助结束日期"
                                fieldDecoratorId="helpEndTime"
                                :labelCol="{ span: 6 }"
                                :wrapperCol="{ span: 12 }">
                    <ta-date-picker style="width: 100%;"
                                    format="YYYY-MM-DD"
                                    :placeholder="disabled?'':'请选择救助结束日期'"
                                    :disabled="disabled"
                ...
                <ta-col :span="24">
                  <ta-form-item label="救助措施"
                                ...
                                :wrapperCol="{ span: 21 }"
                                :initValue="[]"
                                :require="{message:'请选择家救助措施'}">
                    <ta-checkbox-group :options="supportTypeList"
                                       :disabled="disabled"
                                       @change="supportTypes1"
                                       :value="supporcheckList" />
                  </ta-form-item>
                  <!-- <ta-form-item label="救助措施"
                                fieldDecoratorId="helpMeasure"
                                :labelCol="{ span: 3 }"
                  ...
                  <ta-form-item label="其他"
                                ...
                                :wrapperCol="{ span: 9 }"
                                v-if="supportType"
                                fieldDecoratorId="helpMeasureOther"
                                :fieldDecoratorOptions="rules.healthStatusOther">
            ...
            <ta-row ref="ktone">
              ...
              <ta-col :span="11">
                ...
                <ta-form layout="horizontal"
                  ...
                  <poorFamiliesChildrenFather :parentlist.sync='parentlistF'
                                              ...
                                              @setAddressFMNow="setAddressFNow"></poorFamiliesChildrenFather>
              ...
              <ta-col :span="11"
                ...
                <ta-form layout="horizontal"
                  ...
                  <poorFamiliesChildrenFather :parentlist.sync='parentlistM'
                                              :isrequired.sync="isrequired1"
                                              ...
                                              @setAddressFMNow="setAddressMNow"></poorFamiliesChildrenFather>
          ...
          <div class="mars1"
            ...
            <ta-row ref="kttwo">
              ...
              <ta-form layout="horizontal"
                ...
                <poorFamiliesChildrenOtherPeope @otherPother="otherPother"
                                                :ortherRe='ortherRe'
                                                ...
                                                :others="zkvalO"
                                                :heathval="heathvalO"
                                                :serioushval="serioushvalO"></poorFamiliesChildrenOtherPeope>
...
<script>
import poorFamiliesChildrenBaseInfo from './components/poorFamiliesChildrenBaseInfo'
import poorFamiliesChildrenFather from './components/poorFamiliesChildrenFather'
import poorFamiliesChildrenMother from './components/poorFamiliesChildrenMother'
import poorFamiliesChildrenOtherPeope from './components/poorFamiliesChildrenOtherPeope'
...
export default {
  ...
  components: {
    poorFamiliesChildrenBaseInfo,
    Title,
    Divider,
    poorFamiliesChildrenFather,
    poorFamiliesChildrenMother,

    poorFamiliesChildrenOtherPeope,
    AddressComponent
  },
  data () {
    return {
      ...
      rules: {
        ...
        academicStatus: this.verificationRules('家庭经济来源'),
  ...
  activated () {
    ...
    if (this.$route.query.id) {
      ...
      this.bj = true
      this.bigType = this.$route.query.bigType// 暂存0和在册1
      this.nowkeys = this.$route.query.nowkey// 类型 低保儿童1      特困儿童2    建档立卡儿童3

      this.getDetails(this.bigType, this.nowkeys)
      if (this.bigType == '0') {
        this.getChildNum()
      }
    } else {
  ...
  methods: {
    fnClick () {
      this.nowTime = this.statusForm.getFieldValue('helpStartTime')
    },
    ...
    //   }
    // },
    supportTypes1 (supporcheckList) {
      console.log(supporcheckList)
      const num = supporcheckList.indexOf('4')
      if (num == '-1') {
        this.supportType = false
      } else {
        this.supportType = true
    ...
    async getyesNoType () {
      ...
      this.supportTypeList = await this.getDictionaries('OTHERSALVATIONOFUNATTENDED')
    ...
    // ACADEMICSTATUS家庭经济来源
    async getxystatus () {
      const data = await this.getDictionaries('FAMILYSOURCEINCOME')
      this.xystatus = data
    },
    ...
    xyChange (checkedList) {
      console.log(checkedList)
      const num = checkedList.indexOf('5')
      if (num == '-1') {
        this.xyval = '0'
      } else {
        this.xyval = '5'
    ...
    async getDetails (bigType, nowkeys) {
      // xyval statusForm  familyBreadwinner
      //  nowkey类型 低保儿童1      特困儿童2    建档立卡儿童3  // bigType暂存0和在册1
      let url = ''
      const params = {
        id: this.id,
        childType: nowkeys
      ...
      if (bigType == 0) {
        url = '/diBaoChildren/queryById'
      } else {
        url = '/poolChildren/getPoolChildrenById'
      }
      const resobj = await this.post(url, params)
      const data = resobj.data.data // 加载数据 2020年1月9日11:56:07
      if (!data.errors) {
        if (bigType == '0') {
          this.baseForm.setFieldsValue(data)
          data.helpStartTime = data.helpStartTime ? moment(data.helpStartTime, 'YYYY-MM-DD') : null
          data.helpEndTime = data.helpEndTime ? moment(data.helpEndTime, 'YYYY-MM-DD') : null
          if (data.familyBreadwinner) {
            if (data.familyBreadwinner.length == 1) {
              data.familyBreadwinner = [data.familyBreadwinner]
            } else {
              data.familyBreadwinner = data.familyBreadwinner.split(',')
            }
          }
          if (data.helpMeasure) {
            if (data.helpMeasure.length == 1) {
              data.helpMeasure = [data.helpMeasure]
            } else {
              data.helpMeasure = data.helpMeasure.split(',')
            }
          }

          this.statusForm.setFieldsValue(data)
          // 根据身份证号调验重接口带出数据
          ...
          const poolChildrenData = data.poolChildrenVo || {}// 业务信息

          // 是否是监护人控制必填
          this.isphonerule = poolChildrenData.isGuardianOfFather == '1'
          this.isphonerule1 = poolChildrenData.isGuardianOfMother == '1'
          if (poolChildrenData.familyBreadwinner) {
            if (poolChildrenData.familyBreadwinner.length == 1) {
              poolChildrenData.familyBreadwinner = [poolChildrenData.familyBreadwinner]
            } else {
              poolChildrenData.familyBreadwinner = poolChildrenData.familyBreadwinner.split(',')
            }
          }

          if (poolChildrenData.helpMeasure) {
            if (poolChildrenData.helpMeasure.length == 1) {
              poolChildrenData.helpMeasure = [poolChildrenData.helpMeasure]
            } else {
              poolChildrenData.helpMeasure = poolChildrenData.helpMeasure.split(',')
            }
          }
          ...
          if (poolChildrenData.familyBreadwinner && poolChildrenData.familyBreadwinner.indexOf('5') == '-1') { // 判断经济来源中的其他是否显示
            this.xyval = '0'
          } else {
            this.xyval = '5'
          }
          if (poolChildrenData.helpMeasure && poolChildrenData.helpMeasure.indexOf('4') == '-1') { // 判断帮扶措施中的其他是否显示
            this.supportType = false
          } else {
            this.supportType = true
          }
          // this.xyval = poolChildrenData.familyBreadwinner
          // this.childBaseInformationVoId = baseData.id//业务
          ...
          if (poolChildrenData.disabledType) {
            ...
            if (
              poolChildrenData.disabledType[poolChildrenData.disabledType.length - 1] ==
          ...
          //   this.supportType = false
          // }
          poolChildrenData.helpStartTime = poolChildrenData.helpStartTime ? moment(poolChildrenData.helpStartTime, 'YYYY-MM-DD') : null
          poolChildrenData.helpEndTime = poolChildrenData.helpEndTime ? moment(poolChildrenData.helpEndTime, 'YYYY-MM-DD') : null
          await this.$nextTick(() => {
          ...
          if (poolChildrenData.relationship == '99') {
            this.ortherRe = true
          } else {
            this.ortherRe = false
          }
    ...
    async submit (val) {
      ...
      // let suggestData = this.suggestForm.getFieldsValue() // 帮扶建议777
      ...
      if (isPass || (val == 's' && baseData.name)) {
        ...
        const poolChildrenVo = { ...heathData, ...statusData, ...motherData, ...fatherData, ...otherManData }
        // const poolChildrenVo = { ...baseData, ...heathData, ...statusData, ...motherData, ...fatherData, ...otherManData }
        poolChildrenVo.deleteId = this.id
        poolChildrenVo.childType = this.nowkeys
        poolChildrenVo.upLoadPathIds = upload// 上传附件放到业务vo中
        DilemmaChildrenAllVo = {
          childBaseInformationVo,
          poolChildrenVo
        }

        let urls = 'poolChildren/savePoolChildren'
        ...
        if (this.bigType == '1') {
          ...
          ImproperCareChildAllVo.poolChildrenVo.id = this.ywBjId
          ImproperCareChildAllVo.poolChildrenVo.childBaseId = this.baseIdBj
        }
