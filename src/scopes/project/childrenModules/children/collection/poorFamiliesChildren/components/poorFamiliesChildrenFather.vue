    ...
    <ta-col :span="24">
      ...
      <ta-col :span="12">
        <ta-form-item
          ...
          :wrapperCol="{ span: 13 }"
        >
          <ta-radio-group @change="isGuardianchs" :disabled="(this.isGuardiandisabled)">
            <ta-radio
              :value="item.value"
  ...
  methods: {
    ...
    idCardValidate (rule, value, callback) {
      if (this.mustTip) {
        ...
        if (this.cardType == '01') {
          callback(
            this.verificationRulesNotMustFill('证件号码', { type: 'idCard' })
          )
        } else {
    ...
    isGuardianchs(e) {
      ...
      let otherDate = this.thisP.otherManForm.getFieldsValue()
      this.thisP.otherManForm.resetFields()
      this.thisP.otherManForm.setFieldsValue(otherDate)
    },
    changeCardType (v) {
      this.cardType = v
      if (v == undefined) {
    ...
    setAddressFMNow () {
      ...
      this.$nextTick(() => {
        if (this.types == 1) {
          this.$emit('setAddressFNow')
        } else {
          this.$emit('setAddressMNow')
        }
      })
    },
    setAddressFM () {
      this.$nextTick(() => {
        if (this.types == 1) {
          this.$emit('setAddressF')
        } else {
          this.$emit('setAddressM')
        }
      })
    },
    // 获取是否监护人
