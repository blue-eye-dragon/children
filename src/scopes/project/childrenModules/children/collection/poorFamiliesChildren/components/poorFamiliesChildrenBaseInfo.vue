...
export default {
  ...
  activated () {
    switch (this.nowkeys) {
      case 1:
        this.curChildT = '131'// 低
        break
      case 2:
        this.curChildT = '133'// 特
        break
      case 3:
        this.curChildT = '132'// 建
        break
    }
  },
  mounted () {
  ...
  methods: {
    ...
    async idcardrole (rule, value, callback) {
      if (!this.id || value != this.oldIdCard) { // 新增或编辑但身份证号改变时，进行验重操作
        this.idCardValidate(rule, value, callback, this, null, this.curChildT)
      } else { // 否则不进行验重
