...
export default {
  ...
  data () {
    return {
      ...
      disabled: false,
      mixListOptions: {
        mixGetDataListURL: '/DilemmaChildApi/listChange', // '/worker/list', // 数据列表接口，API地址
  ...
  methods: {
    ...
    async over (value, obj, index) {
      ...
      if (index == 'see') {
        this.orSee = true
        const res = await this.post('/DilemmaChildApi/queryById', { id: obj.dilemmaChildId })
        const data = res.data.data.dilemmaChildVo
        if (data) {
          this.$nextTick(() => {
      ...
      this.$nextTick(() => {
        this.form.setFieldsValue({ id: obj.dilemmaChildId })
      })
    },
    async  reOver (value, obj, tp) {
      // 接口未 2020年1月18日15:32:00
      const chenck = await this.post('/DilemmaChildApi/getTroubleChildMsgByChildId', { id: obj.id })
      if (chenck.data.data == 1) {
        ...
        this.$router.push({
          ...
          query: {
            ...
            id: obj.dilemmaChildId,
            disabled: tp,
            url: '/DilemmaChildApi/restoreGuardianship'
          }
        })
      }
    },
    async submit () {
      ...
      const res = await this.post('/DilemmaChildApi/saveExitMsg', param)
      if (!res.errors) {
        this.onClose()
        this.handleSubmit()
      }
