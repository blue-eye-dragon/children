  ...
  <div id="publicRoleManager"
    ...
    <ta-border-layout :layout="{header:'70px',footer:'70px'}"
      ...
      <ta-row slot="header"
        ...
        <ta-input-search placeholder="请输入角色名称"
                         ...
                         @search="onSearchUser"
                         class="search-box">
          <ta-button slot="enterButton"
                     type="primary">搜索</ta-button>
        </ta-input-search>
      </ta-row>
      <div slot="centerExtraContent"
           style="height: 50px;line-height: 48px;">

        <ta-checkbox :checked="isSub"
                     @change="onChangeIsSub">包含子组织</ta-checkbox>
        <ta-tag-select title="状态"
                       :data="[{value:'1',label:'启用'},{value:'0',label:'禁用'}]"
                       v-model="effective"
                       @change="filterClick"></ta-tag-select>
        <ta-tag-select title="可分配该角色到子组织人员"
        ...
        <div style="float: right"
          ...
          <ta-button type="primary"
                     @click="fnNewPublicRole">新增角色</ta-button>
          <ta-dropdown :trigger="['click']">
            <ta-menu slot="overlay">
              <!-- <ta-menu-item>
                <ta-popconfirm title="确实删除选择的角色？"
                               @confirm="deletePublicRole"
                               okText="确定"
                               cancelText="取消">
                  <div>删除</div>
                </ta-popconfirm>
              ...
              <!-- <ta-menu-divider />
              <ta-menu-item key="1"
                            @click="fnBatchRoles(1)">批量授予</ta-menu-item>
              <ta-menu-item key="2"
                            @click="fnBatchRoles(2)">批量收回</ta-menu-item> -->
            </ta-menu>
            <ta-button>
              批量操作
              <ta-icon type="down" />
      ...
      <div class="coneent">
        <div class="lefts">
          <ta-spin class="spinning"
                   :spinning="spinning">
          </ta-spin>
          <ta-e-tree v-show="!spinning2"
                     :load="loadNode"
                     node-key="orgId"
                     :props="defaultProps"
        ...
        <div class="right">
          <ta-table :rowSelection="{selectedRowKeys: selectedRowKeys, onChange: onSelectChange}"
            ...
            <template slot="effective"
              ...
              <ta-tag v-if="text==0"
                      color="red"
                      class="no-cursor">禁用</ta-tag>
              <ta-tag v-else
                      color="green"
                      class="no-cursor">启用</ta-tag>
            </template>
            <span slot="roleSign"
                  slot-scope="text">{{CollectionLabel('ROLESIGN',text)||'--'}}</span>
            <template slot="roleOperation"
                      slot-scope="text,record"
                      v-if="ishide||(!ishide&&record.roleName.indexOf('hyun_通用')==-1)">
            ...
            <template slot="operation"
              ...
              <template v-if="ishide">
                <ta-dropdown :trigger="['click']"
                  ...
                  <ta-menu slot="overlay">
                    <ta-menu-item key="1"
                                  @click="fnRouteToRoleAuthority(record)">功能菜单权限管理</ta-menu-item>
                    <ta-menu-item key="2"
                                  @click="fnRouteToRoleCustomAuthorituy(record)">自定义对象权限管理</ta-menu-item>
                  </ta-menu>
                </ta-dropdown>
                <span v-else
    ...
    <ta-modal title="角色复制"
      ...
      <template slot="footer">
        ...
        <ta-button key="submit"
                   type="primary"
                   @click="fnRoleCopySave">
          保存
        </ta-button>
      </template>

      <roleCopy ref="roleCopyChild"
                v-bind:roleObj="currentSelectedPublicRole"
                v-bind:orgTreeData="orgOptions"
    ...
    <ta-modal title="权限复制"
      ...
      <template slot="footer">
        ...
        <ta-button key="submit"
                   type="primary"
                   @click="fnAuthorityCopySave">
          保存
        </ta-button>
      </template>

      <authorityCopy ref="authorityCopyChild"
                     v-bind:roleObj="currentSelectedPublicRole"></authorityCopy>
    </ta-modal>
    <ta-drawer :title="newRoleTitle"
               placement="right"
               @close="onNewRoleClose"
               :visible="newRoleVisible"
               :destroyOnClose=true
               width="500">
      <newRole v-bind:updateObj="updateObj"
               @onNewRoleClose="onNewRoleClose"
               @fnQueryRolesByOrgId="fnQueryRolesByOrgId"
               ref="newRole"></newRole>
      <div slot="footer">
        <ta-button-group>
          <ta-button @click="$refs.newRole.fnResetForm()">重置</ta-button>
          <ta-button @click="$refs.newRole.fnAddOrUpdateRole()"
...
export default {
  ...
  mounted () {
    this.ishide = window.parent.indexTool.getUserInfo().userId == 1 || window.parent.indexTool.getUserInfo().userId == 421104
  },
  methods: {
    loadNode (node, resolve) {
      ...
      Base.submit(null, {
        ...
        const treeData = result.data.orgVos || []
        if (level == 0) { // 初次加载数据，表格将自动加载最高级
          treeData[0] ? this.fnNodeClick(treeData[0]) : null
        }
        this.spinning = false
        setTimeout(() => {
...
.lefts {
  ...
  padding-right: 10px;
  height: auto;
  border-right: 1px solid rgba(220, 221, 222, 1);
}
.right {
  width: 80%;
  padding-left: 4px;
