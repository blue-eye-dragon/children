  ...
  <div class="fit">
    <ta-border-layout :layout="{header:'55px'}"
      ...
      <div slot="centerExtraContent">
        <ta-alert :message="'当前角色为：'+this.$route.query.roleName"
                  type="info"
                  showIcon />
      </div>
      <ta-tabs class="fit">
          ...
          <ta-row style="width: 100%;padding-top: 10px">
            <ta-col :span="row.col.span">
              <ta-table :columns="orgColumns"
                ...
                <span slot="customOrgNameTitle">
                  <ta-icon type="smile-o" /> Name</span>
        ...
        <template slot="tabBarExtraContent">
          <ta-button @click="fnBackToHome">
            <ta-icon type="rollback" />返回</ta-button>
          <ta-button type="primary"
                     @click="fnShowModal">组织权限管理</ta-button>
        </template>

      </ta-tabs>
    </ta-border-layout>
    <!--begin 权限管理模态框加载-->
    <ta-modal :visible="authority.visible"
              :centered="true"
              :destroyOnClose="true"
      ...
      <!--</div>-->
      <!--</template>-->
      <admin-authority-mg :item="item"
                          @modalCancel="fnReLoadAuthority"></admin-authority-mg>
    </ta-modal>
    <!--end 权限管理模态框加载-->
  </div>
