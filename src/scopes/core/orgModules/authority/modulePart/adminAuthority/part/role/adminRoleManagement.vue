  ...
  <div id="roleManagement"
    ...
    <ta-border-layout :layout="{header:'55px',footer:'70px'}"
      ...
      <div slot="centerExtraContent">
        ...
        <ta-checkbox :checked="includeSub"
        ...
        <div style="float: right">
          <ta-button @click="fnAddAdmin"
                     type="primary">新增角色</ta-button>
          <!--begin 批量操作按钮-->
          <ta-dropdown :trigger="clickTrigger">
            <ta-menu slot="overlay">
              <ta-menu-item :disabled="!selectedRowKeys.length">
                <ta-popconfirm title="确认启用所选管理员角色?"
                               cancelText="取消"
                               okText="确认"
                               @confirm="fnBatchPickAdmin(false)">
              ...
              <ta-menu-divider />
              <ta-menu-item :disabled="!selectedRowKeys.length">
                <ta-popconfirm title="确认禁用所选管理员角色?"
                               cancelText="取消"
                               okText="确认"
                               @confirm="fnBatchBanAdmin(false)">
                  <ta-icon type="stop" />
      ...
      <ta-table :columns="adminColumns"
        ...
        <span slot="expire"
          ...
          <ta-tag v-if="text == '0'"
                  type="success"
                  class='no-cursor'>未过期</ta-tag>
          <ta-tag v-else-if="text == '1'"
                  type="danger"
                  class='no-cursor'>过期</ta-tag>
        </span>
        <!--处理有效标识-->
        <span slot="effective"
              slot-scope="text">{{CollectionLabel('STATE',text)}}</span>
        <span slot="action"
              slot-scope="text,record">
          <ta-table-operate :operateMenu="operateMenu"></ta-table-operate>
        </span>
        <template slot="adminMg"
                  slot-scope="text,record">
          ...
          <span v-if="record.effective == '1'">
            <ta-dropdown :trigger="clickTrigger">
              ...
              <ta-menu slot="overlay">
                ...
                <ta-menu-item>
                  <router-link :to="{ path: 'adminGrantAuthority' , query: {roleName:record.roleName,roleId:record.roleId}}">授权权限</router-link>
                </ta-menu-item>
                <ta-menu-item>
                  <router-link :to="{ path: 'adminOrgAuthority' , query: {roleName:record.roleName,roleId:record.roleId}}">组织范围权限</router-link>
                </ta-menu-item>
              </ta-menu>
            </ta-dropdown>
          </span>
          <span v-else
                title="禁用的管理员角色不允许进行权限管理">
          ...
          <span v-if="record.effective == '1'">
            <ta-dropdown :trigger="clickTrigger">
              ...
              <ta-menu slot="overlay">
                ...
                <ta-menu-item>
                  <router-link :to="{ path: 'adminObjectGrantAuthority' , query: {roleName:record.roleName,roleId:record.roleId}}">自定义授权权限</router-link>
                </ta-menu-item>
              </ta-menu>
            </ta-dropdown>
          </span>
          <span v-else
                title="禁用的管理员角色不允许进行自定义权限管理">
    ...
    <!--使用权限模态框加载-->
    <ta-modal :visible="batch.visible"
              :centered="true"
              :destroyOnClose="true"
              width="800px"
      ...
      <template slot="footer">
        <div style="text-align: center">
          ...
          <ta-button type="primary"
                     @click="fnSaveModalAuthority">保存</ta-button>
        </div>
      </template>
      <ta-row :gutter="10"
              style="width: 100%">
    ...
    <ta-drawer destroyOnClose
      ...
      <div slot="footer">
        <ta-button-group>
          ...
          <ta-button type="primary"
                     @click="fnAddAdminInfo">保存</ta-button>
        </ta-button-group>
      </div>
      <add-admin :admin="adminItem"
                 @closeAdminDrawer="fnCloseAdminDrawer"
