  ...
  <div class="fit">
    <div v-if="state.barType == 'tab'" class="index-tabs" ref="index-tabs">
      <div
              ...
              :style="{width:worktableWidth, color: activeTab=='worktable' ? mainColor: '', 'border-bottom-color': activeTab=='worktable' ? mainColor: ''}"
      >工作台</div>
      <!-- <div class="tab-item"
           @click="showModal"
           >督办</div> -->
      <div
           ...
           :key="menu.id"
           :style="{color: activeTab==menu.id ? mainColor: '', 'border-bottom-color': activeTab==menu.id ? mainColor: ''}"
           :id="menu.id+'-tab'">
        {{menu.name}}
        <span>
          <ta-icon type="close" @click.stop="closeTabFn(menu)" />
        </span>
      </div>
      ...
      <span @click.stop="ifShowMore=ifShowMore?false:true" flag="tab-more">
        <ta-icon type="setting" :style="{color: ifShowMore ? mainColor : ''}"/>
      </span>
        <div v-if="ifShowMore" class="tab-more-list" flag="tab-more">
          <div @click.stop="setTabsted({id:'worktable'})" class="more-worktable" flag="tab-more" :style="{color: mainColor}">工作台</div>
          <div
               class="tab-item"
               v-for="menu in list2"
               :id="menu.id+'-tab'"
          ...
          <div class="close-all" :style="{background: mainColor}" flag="tab-more" @click="closeAllTab()">关闭所有</div>
        </div>
      </div>
    </div>
  ...
  methods: {
    ...
    setTab () {
      if (this.state.barType == 'tab') {
        ...
        let tabsL = parseInt(this.worktableWidth) + 80// 工作台+更多空白
