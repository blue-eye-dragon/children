<template>
  <ta-drawer @close="close"
    ...
    <h4 class="msg-head"
        :class="{'msg-new': newMsgSize>0}">消息（{{this.newMsgSize}}未读）
      <ta-icon type="sync"
               class="refresh"
               @click="refreshMessage" />
      <span style="margin: 0px 20px"
            class="refresh"
            @click="openRecord">查看更多</span>
    </h4>
    <ta-tabs defaultActiveKey="1"
      ...
      <ta-tab-pane tab="通知(未读)"
        ...
        <div class="msg-item new"
          ...
          <ta-checkbox :checked="item.checked"
                       @click="checkNotice(item)"></ta-checkbox>
          <ta-icon type="sound"
                   style="color: #ca1064"
                   v-if="item.type=='01'" />
          <ta-icon type="form"
                   style="color: #04b2dc"
                   v-else-if="item.type.indexOf('02') == 0" />
          <ta-icon type="mail"
                   style="color: #108ee9"
                   v-else-if="item.type=='03'" />
          <ta-icon type="exclamation-circle-o"
                   style="color: red"
                   v-show="item.readSign=='0'" />
      ...
      <ta-tab-pane tab="私信"
                   key="2">
        <div class="msg-item msg-item-letter"
             v-for="item in messageList2"
             :key="item.chatId"
             @click="clickDetail2(item)"
             :class="{'active': item == clickIndex,'new': item.newLetter == '1'}">
          <div class="msg-content">{{item.twoName}}<span class="msg-content-op"
                  v-show="item.chatId!='0'"
                  @click="removeChat(item,$event)">删除</span></div>
        </div>
      </ta-tab-pane>
    </ta-tabs>
    <div class="msg-detail"
      ...
      <div class="msg-detail-foot">
        <ta-button type="primary"
                   @click="clickSend()"
                   v-if="messageType=='03'">回复</ta-button>
        <ta-button type="primary"
                   @click="clickOpen()"
                   v-else-if="messageType.indexOf('02') == 0">进入功能
        </ta-button>
    ...
    <div class="msg-detail"
      ...
      <div class="msg-detail-con">
        <div class="chat-content">
          <div class="chat-detail-item tip"
               v-show="chatId!='0'">
            <span class="chat-history"
                  @click="chatHistory">历史记录</span>
            (只会保存最近7天)
          </div>
          <div v-for="item in chatList"
               :key="item.mId">
            <div class="chat-detail-item you"
                 v-if="item.senderId!=nowUserId||item.owner=='you'">
              <div class="cd-item-time">{{item.sendDate}}</div>
              <div class="cd-item-content-wrapper clearfix">
                <div class="cd-item-avatar-wrapper">
                  <ta-avatar style="backgroundColor:#87d068"
                             size="default">{{item.senderName.substring(0,1)}}
                  </ta-avatar>
                </div>
                <div class="cd-item-content">
                  <i class="cd-item-content-arrow-left"></i>
                  <span class="cd-item-content-text"
                        v-html="item.content">{{item.content}}</span>
                  <span class="cd-item-content-state "></span>
                </div>
              </div>
            </div>
            <div class="chat-detail-item me"
                 v-else>
              <div class="cd-item-time">{{item.sendDate}}</div>
              <div class="cd-item-content-wrapper clearfix">
                <div class="cd-item-avatar-wrapper">
                  <ta-avatar style="color: #f56a00; backgroundColor: #fde3cf"
                             size="default">
        ...
        <ta-textarea class="chat-content-say"
                     placeholder="请输入内容"
                     v-model="content2"
    ...
    <div slot="footer"
      ...
      <ta-button style="padding: 0 5px;"
                 @click="checkNotices()">全选</ta-button>
      <ta-button style="padding: 0 5px;"
                 @click="removeNotices()">删除</ta-button>
      <ta-button style="padding: 0 5px;"
                 @click="readNotices()">标记已读</ta-button>
    </div>
    <div slot="footer"
         v-else-if="tabType=='2'">
      <ta-button type="primary"
                 style="padding: 0px 10px;"
                 @click="clickShowSelectUserModal">添加联系人</ta-button>
      <ta-modal title="人员选择"
                cancelText="取消"
                okText="确认"
                v-model="userModal"
        ...
        <ta-select showSearch
                   ...
                   @change="chaterChange"
                   placeholder="选择人员">
          <ta-select-option v-for="user in userList2"
                            :key="user.userId+':'+user.loginId+':'+user.name">
            {{user.loginId}}@{{user.name}}
          </ta-select-option>
        </ta-select>
      </ta-modal>
    </div>
    <div class="msg-send msg-detail"
         v-if="showSend">
      <span class="detail-title">{{title}}</span>
      <div class="msg-detail-con">
        <ta-form :autoFormCreate="(form)=>{this.form = form}">
          <ta-form-item label="接收人"
                        :labelCol="{ span: 3 }"
                        :wrapperCol="{ span: 21 }">
            <ta-select mode="multiple"
                       ...
                       v-model="selectUsers">
              <ta-select-option v-for="user in userList"
                                :key="user.userId">{{user.loginId}}@{{user.name}}
              </ta-select-option>
            </ta-select>
          </ta-form-item>
          <ta-form-item label="主题"
                        :labelCol="{ span: 3 }"
                        :wrapperCol="{ span: 21 }">
            <ta-input v-model="sendTitle" />
          </ta-form-item>
          <ta-form-item>
            <ta-upload @change="handleUploadChange"
                       name="file"
                       :multiple="true"
                       :action="backUrl+'/message/uploadNoticeFile'"
                       :withCredentials="true"
                       :beforeUpload="beforeUploadNoticeFile"
...
.msg-list {
  ...
  .msg-detail {
    ...
    .msg-detail-detail {
      ...
      overflow: auto;
      > table td {
        //通知查看表格样式 采用wangEditor-container 的
  ...
  .cd-item-content-arrow-right {
    ...
    background: no-repeat
      url(data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAkAAAANCAYAAAB7AEQGAAAAbUlEQVQYlY3NQQqCUBQF0FPrE0JoO06cR0toDw6aNtAtCE5dglARNNBJguj//u7sXc7jHsbiNNpPc0wA+KbQA9ke6pDjHUMfnDFADJVo5yOEelyXRQhdfnNR9MJt/bVGFZ4pdA/Mb1D9D+pDaAKByBPqoGsOoAAAAABJRU5ErkJggg==);
  ...
  .cd-item-content-arrow-left {
    left: -6px;
    background: no-repeat
      url(data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAkAAAANCAYAAAB7AEQGAAAAdklEQVQYlY2QMQrCUBBEn6lEAx7B3ht6CA+TKlWqHMI2ZQ4gaCE8m0Q+nyWbgYFlmF1mFnU0QQN8yaCe1GHrEgsv6jMzod7Ud2ZCve8xHdUpalfiAzzqcge11lpgBs6rUF8CeAFdKUQmgL5+ZsRrGTzK9N9fhx/jSPQOTglOfwAAAABJRU5ErkJggg==);
    background-size: contain;
