...
const indexStore = {
  ...
  getters: {
    ...
    getThemeColor: (state) => {
      ...
      return themeList && theme ? themeList[theme]['primary-color'] : '#1890FF'
    },
  ...
  actions: {
    ...
    async setUserInfo ({ commit }) {
      await Base.submit(null, {
        url: 'user/getCurUserAccount', // indexRestService
  ...
  mutations: {
    ...
    _setUserInfo (state, value) {
      ...
      localStorage.setItem('loginId', state.userInfo.loginId)
      // 处理同浏览器不同页面登录问题 end
    },
